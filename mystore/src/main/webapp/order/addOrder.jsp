...
$(document).ready(function(){
	...
	$("#add").click(function (e){
		...
		$("#items").append('<div class="items">Product ID: <br><select class="autocomplete form-control select" name="prod" required>' + 
				...
				'<option value="<c:out value="${product.sku}"/> <c:out value="${product.unitPrice}"/>"><c:out value="${product.name}"/> </option></c:forEach> </select> <br><br>' +
				'Quantity :<br> <input type="text" class="form-control" name="orderProdQuantity" pattern="^[0-9]*$" title="Number only" required value="0"> 
				<input type="button" value="Delete" id="delete" class="btn btn-danger align-right">
				</div><br><br>');
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
          ...
          <div class="row">
			<div class="mx-auto" style="width: 400px;">
			 <form action = "/mystore/OrderController" method = "post">
				...
				<c:forEach items="${products}" var="product">
					<option value="<c:out value="${product.sku}"/> <c:out value="${product.unitPrice}"/>"><c:out value="${product.name}"/>  </option>
				</c:forEach>
				</select> <br><br>
				Quantity :<br>
				<input type = "text"  class="form-control" name="quantity" pattern="^[0-9]*$" title="Number only" required><br>
				</div>
				</div>
